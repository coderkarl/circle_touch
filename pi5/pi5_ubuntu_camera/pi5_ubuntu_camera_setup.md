...
This section differs by distro:
...
`rpicam-apps` and `python3-picamera2` are generally not available from standard Ubuntu repos.
Use `libcamera` + OpenCV instead:

```bash
sudo apt update
sudo apt install -y libcamera-tools python3-libcamera python3-opencv v4l-utils
```

For some Arducam CSI modules, Ubuntu also needs Arducam's driver/overlay install for your exact sensor model.
...
libcamera-still -o ~/cam_test.jpg --nopreview
```

If `libcamera-still` is not present in your Ubuntu image, use:

```bash
cam -l
```

Copy test image to laptop and verify.
...
#### Ubuntu on Raspberry Pi 5 (OpenCV smoke test)
...
if not cap.isOpened():
	raise SystemExit('Could not open camera via /dev/video0')
...
print('Saved opencv_test.jpg', frame.shape)
PY

ls -lh ~/opencv_test.jpg
```

Expected: `opencv_test.jpg` created.
...
dmesg | grep -Ei 'csi|camera|imx|ov|unicam|arducam'
```

If the camera is not enumerated, verify your exact Arducam sensor model and apply the required Ubuntu overlay/driver package from Arducam for that module.
